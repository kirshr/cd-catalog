...
$add_album = $conn->prepare
            ("INSERT INTO album (name, genre_id, label_id, artist_id, description, year, image) 
            VALUES (?,?,?,?,?,?,?)");
...
$get_all_albums = $conn->prepare("SELECT album.name, album.year, artist.name AS ArtistName, genre.name AS GenreName, label.name AS RecordName, album.description, image
FROM album 
INNER JOIN artist 
